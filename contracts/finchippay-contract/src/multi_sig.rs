...
use crate::storage::*;
    ...
    pub fn multisig_count(env: Env) -> u32 {
        { let key = DataKey::MultiSigCount; let c = env.storage().persistent().get(&key).unwrap_or(0); bump_if_present(&env, &key); c }
    }
    ...
    pub fn get_contract_stats(env: Env) -> (u32, u32, u32) {
        (
            { let key = DataKey::EscrowCount; let c = env.storage().persistent().get(&key).unwrap_or(0); bump_if_present(&env, &key); c },
            { let key = DataKey::StreamCount; let c = env.storage().persistent().get(&key).unwrap_or(0); bump_if_present(&env, &key); c },
            { let key = DataKey::MultiSigCount; let c = env.storage().persistent().get(&key).unwrap_or(0); bump_if_present(&env, &key); c },
        )
    }
